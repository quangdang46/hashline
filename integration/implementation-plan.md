...
> **Formatting note that affects the repo:** `.gitignore` line 9 ignores `*.md` and line 13 ignores `.tmp*`. There is **no `integration/` rule**, so the `integration/` directory itself is trackable — but **any `*.md` file under `integration/` will be silently gitignored** (the same applies to `README.md`). Plan accordingly:
> - Commit install/usage docs as `.md` by adding an explicit exception to `.gitignore` (task E.16): `!integration/**/*.md` or `!integration/**`.
> - OR name per-package docs `README.txt`/`SETUP.txt` until a `.gitignore` change lands.
> The rest of this plan assumes the `.gitignore` exception is added.
...
Resolution order, in `src/hashline.ts` / `src/lib/binary.ts`:
...
The wrappers depend on this CLI contract. It is verified against source (`crates/core/src/cli.rs`, `commands/read.rs`, `commands/patch.rs`). **Treat this table as the compatibility contract; bump `MIN_HASHLINE_VERSION` (constant in each package) if any shape changes.**
...
| `hashline patch <file> <patch>` | `--dry-run`, `--json`, `--safe` | `[<path>#<4hex>]` + updated `N:hh\|content` lines; with `--json`: `{"success": bool, "dry_run": bool, "file": string, "edits_applied": number, "diff": [...]}` | parser warnings `warning: ...`; errors `Error: ...\nHint: ...` or JSON object | 0 ok (incl. `*** Abort` no-op), 1 error (stale anchor, empty patch, binary file, missing file) |
...
**Decision (both wrappers):** render and accept anchors in the **binary's native format**. For pi, that is `N:hh|content` (exactly what `hashline read` prints), and edit ops accept `pos`/`end` anchors of the form `N:hh`. For OpenCode, `hashline_read` emits `LINE#HASH:content` **only if** the wrapper transposes the binary's `N:hh|content` — the plan standardizes on the **binary's `N:hh|content`** to avoid introducing a third incompatible format and to keep the edit anchor grammar identical to the binary's. The OpenCode prompt and zod `.describe()` strings advertise `N:hh|content` and `N:hh` anchors (NOT the paulp-o `N#ID` / tianhuil `N:HH|content` shapes).

> **[VERIFY] Anchor separator for the OpenCode wrapper:** paulp-o uses `LINE#HASH:content` (`TAG_RE`, `.tmp/hashline-edit-opencode/src/lib/hashline-core.ts:99`), tianhuil uses `N:HH|content` (`.tmp/opencode-hashlines/src/lib/hashline.ts:58`), and the binary uses `N:hh|content`. The plan chooses binary-native. Confirm no user-facing docs in this repo (AGENTS.md, README.md, `hashline-ecosystem-research.md`) already committed to `LINE#HASH` — AGENTS.md shows `12:ab3f` and `N:hh|content` (lines 20-25), so binary-native is consistent with this repo's docs.
...
- `n` is 1-based; content is LF-normalized, BOM-stripped text (`document.rs:60-70`). The trailing newline of the file does NOT produce a phantom line in `read --json` (`read.rs:22-23` filter). CRLF files appear LF-only in the JSON.
...
- **`read` has NO `--offset`/`--limit`/`--anchor` flags** (cli.rs:33-39 — only `--json`, `--no-cache`). `read` always emits the whole file. [VERIFY] If pagination is required, the wrapper slices the returned lines in TS (offset/limit are wrapper-only parameters). The repo's `find-block` subcommand is the only line-scoping primitive.
...
| empty / garbage-only patch | 1 | (empty) | `Error: patch produced no edits — ...` / EmptyPatch |
| binary file / missing file / I/O | 1 | (empty) | `Error: ...` + Hint |
| ambiguous hash / hash not found | 1 | (empty) | `Error: ...` + Hint |

There is **no dedicated stale/no-op code**; exit 1 is "any HashlineError". The wrapper maps exit 1 + stderr text to a structured tool error with a `kind` field (see D.2) and appends the "re-read" teaching text to the model-facing message.

### A.7 What the Rust side needs to expose

**Nothing new today.** `crates/core/src/mcp.rs` already exposes read/patch/write/find_block/remove_file/rename_file as JSON-RPC tools, and the CLI exposes the same operations. The wrappers depend only on the CLI contract in A.3/A.5. **No Rust changes are required for the initial release.**

Documented in D.4 as optional future work:
- `hashline grep` subcommand (does NOT exist today) — the OpenCode `hashline_grep` tool must spawn `rg` itself and re-hash lines by piping through the binary, or defer (see C.6).
...
pi-coding-agent extension. The `pi` runtime loads `index.ts` directly (no compile step; `package.json` `"pi": {"extensions": ["./index.ts"]}`). TypeScript is run through pi's Bun/Node loader, so source ships uncompiled — mirror pi-hledit's `"main": "index.ts"` + `"type": "module"` shape (pi-hledit has `"type": "module"`; pi-hashline-edit does not — either works, but **use `"type": "module"`** like the newer pi-hledit package).
...
- Biome (dev-dep) is used for lint; contract tests use `node:test` + `tsc -p tsconfig.contract.json` (the pi-hledit strategy), which suits a thin wrapper — unit-test arg translation + stdout parsing, not the live binary. (Vitest is also present per the task's requested deps; see B.8 for the recommended split.)
...
│   ├── grep.ts                # OPTIONAL: rg --json | hashline ripgrep-annotate? No binary grep today
│   │                          #   → plan: spawn rg, re-hash matched lines via hashline read of the file,
│   │                          #   or defer (see B.7)
...
export default function (pi: ExtensionAPI): void {
  ...
  registerEditTool(pi);
  if (getGrepEnabled()) {
    // registerGrepTool(pi); // optional, B.7
  }
...
Mirror of `.tmp/pi-hashline-edit/index.ts:7-28`. `ExtensionAPI` and `ExtensionFactory` (`types.d.ts:1104`) are the real types; the default export is a plain `(pi: ExtensionAPI) => void`.
...
export function registerReadTool(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      const file = resolveToCwd(params.path, ctx.cwd);        // expand ~, join cwd
...
- `offset`/`limit` are wrapper-side slices of the whole-file JSON (the binary has no pagination flags — [VERIFY] cli.rs:33-39). Emit `details.nextOffset` when truncated so the model knows to continue.
- Errors from `hashline read` (missing/binary file → exit 1) are surfaced as `Error: ...` content with `isError: true`, mirroring `pi-hashline-edit/src/read.ts:162-207` but sourced from the binary's stderr rather than TS `fs` checks.
...
const replaceEdit = Type.Object({
  ...
  pos: Type.String({ description: "anchor N:hh from read" }),
  end: Type.Optional(Type.String({ description: "inclusive end anchor N:hh of the range" })),
  lines: Type.Array(Type.String()),
}, { additionalProperties: false });

const appendEdit  = Type.Object({ op: literalString("append"),  pos: Type.Optional(Type.String()), lines: Type.Array(Type.String()) });
const prependEdit = Type.Object({ op: literalString("prepend"), pos: Type.Optional(Type.String()), lines: Type.Array(Type.String()) });

export const editToolSchema = Type.Object({
  path: Type.String(),
  edits: Type.Array(Type.Union([replaceEdit, appendEdit, prependEdit])),
}, { additionalProperties: false });
```
...
| `delete {pos}` (task-requested op) | `DEL N:hh` | delete single line |
...
async execute(_toolCallId, params, signal, _onUpdate, ctx) {
  ...
  const patchText = buildPatchText(params.edits);          // edit-args.ts
  const { stdout, stderr, exitCode } = await runHashline(
...
- **Chained edits:** after a successful patch, run `hashline read <file> --json` and append the re-rendered anchor block (or an `--- Anchors ---` section) so the model can continue editing without a separate read call. This is the wrapper analog of pi-hashline-edit's `--- Anchors A-B ---` block (`edit-response.ts:62-136`) — the anchors come from the binary, not from TS computation.
- **No-op detection:** the binary handles `*** Abort` (exit 0, no output) and empty/garbage patches (exit 1, EmptyPatch). The wrapper does not maintain its own no-op loop guard (binary authoritative); optional [VERIFY] if the host needs one, track it in `result.ts`.
...
- **hashLength semantics change:** the binary computes 2-hex hashes and the wrapper cannot re-hash. For v0.1.0: accept the field but treat values `3|4` as advisory; the anchors rendered come from the binary regardless. **[VERIFY]** Whether `hashline` will ever add an adaptive-length flag (`A.3`/`hashline-ecosystem-research.md:118-121` cites the idea) — if it lands, pass `--hash-length N` to read/patch and the config becomes functional again. For now document the limitation.
...
Verified against the installed SDK `C:/Users/ADMIN/.config/opencode/node_modules/@opencode-ai/plugin/dist/index.d.ts` (v1.4.6) and `dist/tool.d.ts`:
- There is **no `helper.tool` / `helper.session` / `helper.user` / `helper.app` / `helper.project`** export. Do not design around it.
- The real surface is:
  - `import type { Plugin } from "@opencode-ai/plugin";`
  - `import { tool } from "@opencode-ai/plugin";` — `tool()` returns a `ToolDefinition` plain object `{ description, args, execute }`; `tool.schema` IS zod (`dist/tool.js`: `tool.schema = z`).
...
├── src/
│   ├── index.ts                # `const plugin: Plugin = async (ctx) => ({ tool: {...},
│   │                           #   "experimental.chat.system.transform": ... }); export default plugin;`
│   ├── lib/
│   │   ├── binary.ts           # resolveHashlineBin(env HASHLINE_BIN, PATH) + Bun.spawn helper
│   │   ├── read.ts             # hashline_read args → `hashline read <path> --json`; render N:hh|content
│   │   ├── edit.ts             # hashline_edit args → `hashline edit`-style JSON → `hashline patch`; parse ok/error
│   │   ├── grep.ts             # hashline_grep: rg --json | re-hash via binary; or deferred (C.6)
│   │   ├── prompt.ts           # renderHashlineEditPrompt port — example hashes regenerated by the binary
│   │   └── mismatch.ts         # thin adapter: parse binary error text; surface remaps if present
...
**Explicitly NOT ported** (all binary-owned): `hashline-core.ts` (computeLineHash/NIBBLE_STR/DICT/formatHashLines/parseTag), `hashline-apply.ts` (applyHashlineEdits/collectEdits/dedup/validateAllHashes/sortEditsBottomUp/detectNoOp), `hashline-strip.ts`, `grep-search.ts`'s `fsBasedSearch`, and opencode-hashlines' `lib/hashline.ts` + `lib/schema.ts`. Keep only: zod arg schemas, prompt text, binary-spawn glue, error rendering.

### C.4 Entry — `src/index.ts`

```ts
import type { Plugin } from "@opencode-ai/plugin";
import { tool } from "@opencode-ai/plugin";
import { buildReadResult, buildEditResult } from "./lib/read-edit"; // etc.
import { renderHashlineEditPrompt } from "./lib/prompt";
...
hashline_read: tool({
  ...
  async execute(args, context) {
    ...
    const { stdout, stderr, exitCode } = await spawnHashline(["read", filePath, "--json"], undefined, context.abort);
...
- Output format: `N:hh|content` lines (binary-native, per A.3 decision). Optionally prefix with `[path#4hex]` — for OpenCode the 4-hex file tag is a useful cheap version marker; include it.
...
hashline_edit: tool({
  ...
  async execute(args, context) {
    ...
    const patchText = buildPatchText(args.edits);            // shared translator (reuse pi-hashline src/edit-args.ts logic)
    const { stdout, stderr, exitCode } = await spawnHashline(
    ...
    if (exitCode !== 0) {
      ...
      return formatMismatch(stderr, args.path);              // includes >>>-style context if binary emits it
    }
...
- `src/lib/binary.ts` accepts an injectable `spawn` (or the tests set `process.env.HASHLINE_BIN` to a fixture stub that reads args and prints canned stdout/exit codes).
...
- Shared helper (`src/hashline.ts` / `src/lib/binary.ts`): `resolveHashlineBin(): string` from `HASHLINE_BIN` env → PATH; on Windows resolve `hashline.exe` (append `.exe` via PATHEXT-style probe). Spawn with `{ cwd: ctx.directory || process.cwd(), stdio: ["pipe","pipe","pipe"] }` **without a shell** to avoid quoting/globbing.
...
|---|---|---|
| `stale_anchor` | exit 1 + stderr contains "changed since last read" or `StaleAnchor` | binary `Error:` text + `Re-read and retry with fresh anchors.` |
| `noop` / `empty_patch` | exit 1 + "produced no edits" / EmptyPatch | binary text + `Patch was empty — nothing to do.` |
| `ambiguous_hash` | exit 1 + "ambiguous" / "multiple matches" | binary text + `Re-read; use the exact N:hh.` |
...
Implementation: `formatHashlineError(stderr: string, exitCode: number): { text: string; kind: ErrorKind }` in each package's `src/result.ts` / `src/lib/mismatch.ts`. Model-facing guidance always lives in the returned `text`; `kind` (and details) are host-side.
...
Pi's native edit dialect uses `{path, edits:[{oldText,newText}]}`, `file_path` alias, and JSON-string `edits`. The reference normalizes these in `prepareArguments` (`edit-normalize.ts:167-212`, `edit.ts:534-541`). For v0.1.0 the override edit tool publishes ONLY the hashline schema (models are steered by `promptSnippet`/`promptGuidelines`); dialect folding is a fast-follow if models regress. [VERIFY] Watch real usage.
...
- **AGENTS.md:** add a short "Editor integrations" note pointing at `integration/`, and correct the [VERIFY] anchor-format inconsistency (AGENTS.md shows `12:ab3f` in prose but `N:hh|content` in samples) to the single binary-native format.
...
| # | Task | File(s) | Effort |
|---|---|---|---|
| E.1 | Add `.gitignore` exception for `integration/` (or commit docs as `.txt`). | `.gitignore` | S |
...
| E.8 | `src/edit-args.ts`: op→patch translation table + envelope; unit tests | `src/edit-args.ts`, `test/edit-args.test.ts` | M |
...
| E.16 | `src/lib/binary.ts`: `resolveHashlineBin` + `Bun.spawn` helper with injectable spawn seam; tests | `src/lib/binary.ts`, `src/tests/binary.test.ts` | M |
| E.17 | `src/lib/read.ts`: `hashline_read` tool + `formatRead` (N:hh\|content, `[path#4hex]` header, slice by offset/limit) | `src/lib/read.ts` | M |
| E.18 | `src/lib/edit.ts`: `hashline_edit` tool + `buildPatchText` (reuse E.8 logic), `context.metadata({title})`, exit-1 → `formatMismatch`; tests | `src/lib/edit.ts`, `src/tests/edit.test.ts` | L |
| E.19 | `src/lib/grep.ts`: `hashline_grep` (deferred by default; rg + binary re-hash path if implemented) | `src/lib/grep.ts` | M |
| E.20 | `src/lib/prompt.ts`: port `renderHashlineEditPrompt`; regenerate example anchors against the real binary | `src/lib/prompt.ts`, `src/tests/prompt.test.ts` | M |
| E.21 | `src/index.ts`: assemble plugin (default export), `tool` map, `"experimental.chat.system.transform"` hook | `src/index.ts` | M |
| E.22 | `src/cli/hread.ts` + `src/cli/hedit.ts` (optional bin wrappers) | `src/cli/*.ts` | S |
| E.23 | `README.md` with `opencode.json` `plugin` array + `agent.build.tools.edit:false` snippet + `HASHLINE_BIN`; optional e2e test gated on real binary | `README.md`, `src/tests/e2e.test.ts` | M |
...
- **Host API version drift.**
  ...
  - OpenCode: references tested against `@opencode-ai/plugin` 1.2.6/1.2.11; installed SDK 1.4.6. The `Plugin`/`tool`/`Hooks` contract is unchanged (verified against 1.4.6), but pin devDep to the built-against version and re-verify `tool.schema`/`ToolContext` shape. **Highest drift risk: `helper.*` — it does not exist; using it would fail at runtime.**
...
- **Hash-format divergence within hex.** `read --json` uses `{"n","hash","content"}`; `patch --json` success uses `{"line","hash","content"}` and appends a phantom trailing empty line (`patch.rs:101-124`). Mitigation: separate parsers per command; never round-trip `patch --json` lines into `read --json` shape (pitfall documented in research).
...
- **No binary grep / pagination today.** `hashline_grep` and read offset/limit are wrapper-side (rg + slice) or deferred. Mitigation: explicitly defer; document `find-block` as the scope primitive.
- **MCP vs CLI contract split.** The MCP server and the CLI can drift (e.g. `find_block` MCP tool has no `json` arg while CLI `find-block --json` does). Wrappers depend on the CLI only; pin against golden fixtures, not MCP docs.
...
- **[VERIFY]** Does the model need `replace_text`/`oldText` dialect folding in pi? Deferred (D.6); add `prepareArguments` folding if real usage shows the model emitting native edit dialect.
...
- **GitHub workflows:** `.github/workflows/` exists (per repo layout). Adding npm-package CI (`pnpm`/`bun` for the two packages) is a follow-up, not in scope here.
...
| Contract | File | Notes |
|---|---|---|
| CLI commands + flags | `crates/core/src/cli.rs` | read has only `--json`/`--no-cache`; NO offset/limit |
| read output | `crates/core/src/commands/read.rs` | `N:hh|content`, `--json` shape, no phantom trailing line |
| patch resolution + exit semantics | `crates/core/src/commands/patch.rs` | `-`/`@path`/literal; `*** Abort` exit 0; `--json` phantom line |
...
| hash format | `crates/core/src/hash.rs` | xxh32(seed 0) low byte → 2 hex; xxh3 top 16 bits → 4 hex; `trim_end` |
| document normalization | `crates/core/src/document.rs` | LF-normalize, BOM-strip, trailing_newline |
| error taxonomy | `crates/core/src/error.rs` | `StaleAnchor`, `EmptyPatch`; exit-1 mapping |
| stderr JSON errors | `crates/core/src/output.rs` | `{"error","hint","command":null}` on stderr only |
| MCP tools (reference contract) | `crates/core/src/mcp.rs` | 6 tools + `hashline_` aliases; result `{content:[{type:"text",text}]}` |
| pi API types | `.tmp/pi-types` → `.../extensions/types.d.ts` | `ExtensionAPI` (registerTool/registerCommand/on), `ToolDefinition` (:343-376), `ExtensionContext` (:209-249), `ExtensionFactory` (:1104) |
| pi thin-wrapper precedent | `.tmp/pi-hledit/index.ts` | runHledit spawn (:232-265), `resolveHleditBin` (:228-230), batch translation |
| pi full-reimpl precedent | `.tmp/pi-hashline-edit/` | schemas (`edit.ts:56-170`), read render (`read.ts:131-252`), config, prompts, error codes |
| OpenCode SDK | `~/.config/opencode/node_modules/@opencode-ai/plugin/dist/index.d.ts`, `tool.d.ts` | `Plugin`, `PluginInput`, `Hooks.tool`, `tool()`, `tool.schema`, `ToolContext` |
