...
fn bench_verify_scaling(c: &mut Criterion) {
    ...
    for count in [1, 10, 100, 1_000] {
        ...
        let anchors: Vec<String> = entries
            ...
            .map(|(index, entry)| {
                format!(
                    "{}:{}",
                    index + 1,
                    hashline::hash::format_short_hash(entry.short_hash)
                )
            })
            .collect();
        ...
        group.bench_with_input(
            ...
            &(entries, fc, anchors),
            |b, (entries, fc, anchors)| {
                b.iter(|| black_box(count_resolve_successes(entries, fc, anchors)))
            },
        );
    }
    group.finish();
