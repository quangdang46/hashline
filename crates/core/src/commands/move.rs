...
pub fn run<W: Write, E: Write>(
        ...
        && cmd.expect_inode.is_none()
    {
        ...
        if let (Some((src, h)), Some((tgt, _))) = (
            try_parse_line_anchor(&cmd.anchor),
            try_parse_line_anchor(&cmd.target),
        ) {
            let r = crate::fast::run_fast_move(
                ctx,
                &cmd.file,
                src,
                tgt,
                h,
                matches!(cmd.direction, crate::cli::MoveDirection::Before),
                cmd.dry_run,
                cmd.expect_mtime,
                cmd.expect_inode,
                false,
